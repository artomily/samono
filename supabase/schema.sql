...
create table if not exists public.wallet_connections (
  ...
  wallet_type     text        not null check (wallet_type in ('metamask', 'binance', 'trust', 'okx', 'other')),
...
begin
  ...
  values (
    new.id,
    -- Wallet-based accounts (email ends in @wallet.bnb) start with null username
